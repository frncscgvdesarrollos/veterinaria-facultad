 ...
 */ function tryGetPreferRestEnvironmentVariable() {
    ...
    if (rawValue === undefined) {
        ...
        console.warn("An unsupported value was specified for the environment variable FIRESTORE_PREFER_REST. Value ".concat(rawValue, " is unsupported."));
        return undefined;
    }
}
/**
 ...
 */ function validateHost(arg, value, options) {
    if (!validateOptional(value, options)) {
        ...
        const urlString = "http://".concat(value, "/");
        let parsed;
        try {
 ...
 */ class ExponentialBackoff {
     ...
     */ backoffAndWait() {
        ...
        if (this.currentBaseMs > 0) {
            (0, logger_1.logger)('ExponentialBackoff.backoffAndWait', null, "Backing off for ".concat(delayWithJitterMs, " ms ") + "(base delay: ".concat(this.currentBaseMs, " ms)"));
        }
 ...
 */ class RateLimiter {
     ...
     * @private
     * @internal
     */ tryMakeRequest(numOperations) {
        let requestTimeMillis = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : Date.now();
     ...
     * @private
     * @internal
     */ getNextRequestDelayMs(numOperations) {
        let requestTimeMillis = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : Date.now();
        this.refillTokens(requestTimeMillis);
        if (numOperations < this.availableTokens) {
            return 0;
    ...
    calculateCapacity(requestTimeMillis) {
        ...
        if (operationsPerSecond !== this.previousCapacity) {
            (0, logger_1.logger)('RateLimiter.calculateCapacity', null, "New request capacity: ".concat(operationsPerSecond, " operations per second."));
        }
 ...
 */ function detectValueType(proto) {
    ...
    if (proto.valueType) {
        ...
        if (detectedValues.length !== 1) {
            throw new Error("Unable to infer type value from '".concat(JSON.stringify(proto), "'."));
        }
 ...
 */ function validateResourcePath(arg, resourcePath) {
    if (typeof resourcePath !== 'string' || resourcePath === '') {
        throw new Error("".concat((0, validate_1.invalidArgumentMessage)(arg, 'resource path'), " Path must be a non-empty string."));
    }
    if (resourcePath.indexOf('//') >= 0) {
        throw new Error("".concat((0, validate_1.invalidArgumentMessage)(arg, 'resource path'), " Paths must not contain //."));
    }
}
/**
 ...
 */ function validateFieldPath(arg, fieldPath) {
    ...
    if (typeof fieldPath !== 'string') {
        throw new Error("".concat((0, validate_1.invalidArgumentMessage)(arg, 'field path'), " Paths can only be specified as strings or via a FieldPath object."));
    }
    if (fieldPath.indexOf('..') >= 0) {
        throw new Error("".concat((0, validate_1.invalidArgumentMessage)(arg, 'field path'), ' Paths must not contain ".." in them.'));
    }
    if (fieldPath.startsWith('.') || fieldPath.endsWith('.')) {
        throw new Error("".concat((0, validate_1.invalidArgumentMessage)(arg, 'field path'), ' Paths must not start or end with ".".'));
    }
    if (!FIELD_PATH_RE.test(fieldPath)) {
        throw new Error("".concat((0, validate_1.invalidArgumentMessage)(arg, 'field path'), ' Paths can\'t be empty and must not contain\n    "*~/[]".'));
    }
 ...
 */ function validateUserInput(arg, value, desc, options, path, level, inArray) {
    if (path && path.size - 1 > MAX_DEPTH) {
        throw new Error("".concat((0, validate_1.invalidArgumentMessage)(arg, desc), " Input object is deeper than ").concat(MAX_DEPTH, " levels or contains a cycle."));
    }
    level = level || 0;
    ...
    if (Array.isArray(value)) {
        ...
        if (options.allowUndefined && level === 0) {
            throw new Error("".concat((0, validate_1.invalidArgumentMessage)(arg, desc), ' "undefined" values are only ignored inside of objects.'));
        } else if (!options.allowUndefined) {
            throw new Error("".concat((0, validate_1.invalidArgumentMessage)(arg, desc), ' Cannot use "undefined" as a Firestore value').concat(fieldPathMessage, ". ") + 'If you want to ignore undefined values, enable `ignoreUndefinedProperties`.');
        }
    } else if (value instanceof field_value_1.VectorValue) {
    // OK
    } else if (value instanceof field_value_1.DeleteTransform) {
        if (inArray) {
            throw new Error("".concat((0, validate_1.invalidArgumentMessage)(arg, desc), " ").concat(value.methodName, "() cannot be used inside of an array").concat(fieldPathMessage, "."));
        } else if (options.allowDeletes === 'none') {
            throw new Error("".concat((0, validate_1.invalidArgumentMessage)(arg, desc), " ").concat(value.methodName, "() must appear at the top-level and can only be used in update() ") + "or set() with {merge:true}".concat(fieldPathMessage, "."));
        ...
        if (inArray) {
            ...
            throw new Error("".concat((0, validate_1.invalidArgumentMessage)(arg, desc), " ").concat(value.methodName, "() can only be used in set(), create() or update()").concat(fieldPathMessage, "."));
        }
    } else if (value instanceof path_1.FieldPath) {
        throw new Error("".concat((0, validate_1.invalidArgumentMessage)(arg, desc), ' Cannot use object of type "FieldPath" as a Firestore value').concat(fieldPathMessage, "."));
 ...
 */ function validateArrayElement(arg, value, allowUndefined) {
    if (Array.isArray(value)) {
        throw new Error("".concat((0, validate_1.invalidArgumentMessage)(arg, 'array element'), " Nested arrays are not supported."));
    }
 ...
 */ class VectorQuery {
     ...
     */ startAfter() {
        ...
        for(var _len = arguments.length, fieldValuesOrDocumentSnapshot = new Array(_len), _key = 0; _key < _len; _key++){
            fieldValuesOrDocumentSnapshot[_key] = arguments[_key];
        }
...
exports.ATTRIBUTE_SETTINGS_PREFIX = "".concat(exports.ATTRIBUTE_SERVICE_PREFIX, ".settings");
...
class QueryUtil {
    _getResponse(query, transactionOrReadTime) {
        let retryWithCursor = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : true, explainOptions = arguments.length > 3 ? arguments[3] : void 0;
    ...
    _stream(query, transactionOrReadTime) {
        let retryWithCursor = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : true, explainOptions = arguments.length > 3 ? arguments[3] : void 0;
        ...
        const stream = new stream_1.Transform({
            ...
            transform: (proto, enc, callback)=>{
                ...
                if (proto.done) {
                    ...
                    this._firestore._traceUtil.currentSpan().addEvent("Firestore.".concat(methodName, ": Received RunQueryResponse.Done."));
            ...
            do {
                ...
                backendStream.on('error', (err)=>{
                    ...
                    if (!isExplain && !transactionOrReadTime && !this._isPermanentRpcError(err, methodName)) {
                        ...
                        this._firestore._traceUtil.currentSpan().addEvent("".concat(trace_util_1.SPAN_NAME_RUN_QUERY, ": Retryable Error."), {
                        ...
                        stream.write(constants_1.NOOP_MESSAGE, ()=>{
                            if (this._hasRetryTimedOut(methodName, startTime)) {
                                ...
                                (0, logger_1.logger)('QueryUtil._stream', tag, 'Query failed with retryable stream error however either retryWithCursor="'.concat(retryWithCursor, '", or ') + 'no progress was made receiving documents, so the stream is being closed.');
                                stream.destroy(err);
                                streamActive.resolve(/* active= */ false);
                            }
                        });
                        ...
                        this._firestore._traceUtil.currentSpan().addEvent("".concat(trace_util_1.SPAN_NAME_RUN_QUERY, ": Error."), {
                            'error.message': err.message
                        });
                        stream.destroy(err);
                        streamActive.resolve(/* active= */ false);
 ...
 */ class Aggregate {
     ...
     */ toProto() {
        ...
        if (this.aggregateType === 'count') {
            ...
            throw new Error("Aggregate type ".concat(this.aggregateType, " unimplemented."));
        }
     ...
     */ _stream(transactionOrReadTime, explainOptions) {
        ...
        firestore.initializeIfNeeded(tag).then(async ()=>{
            ...
            backendStream.on('error', (err)=>{
                ...
                this._query._firestore._traceUtil.currentSpan().addEvent("".concat(trace_util_1.SPAN_NAME_RUN_AGGREGATION_QUERY, ": Error."), {
                    'error.message': err.message
                });
                stream.destroy(err);
            });
     ...
     */ decodeResult(proto) {
        ...
        if (fields) {
            ...
            for (const prop of Object.keys(fields)){
                ...
                assert(alias !== null && alias !== undefined, "'".concat(prop, "' not present in server-client alias mapping."));
                if (this._aggregates[alias] === undefined) {
                    throw new Error("Unexpected alias [".concat(prop, "] in result aggregate result"));
                }
        ...
        const runQueryRequest = {
            ...
            structuredAggregationQuery: {
                ...
                aggregations: (0, util_1.mapToArray)(this._aggregates, (aggregate, clientAlias)=>{
                    ...
                    assert(serverAlias !== null && serverAlias !== undefined, "'".concat(clientAlias, "' not present in client-server alias mapping."));
    ...
    _query, _aggregates){
        ...
        for(const clientAlias in this._aggregates){
            if (Object.prototype.hasOwnProperty.call(this._aggregates, clientAlias)) {
                const serverAlias = "aggregate_".concat(aggregationNum++);
 ...
 */ class Query {
     ...
     */ _parseFieldFilter(fieldFilterData) {
        ...
        if (index_1.FieldPath.documentId().isEqual(path)) {
            if (operator === 'array-contains' || operator === 'array-contains-any') {
                throw new Error("Invalid Query. You can't perform '".concat(operator, "' ") + 'queries on FieldPath.documentId().');
            } else if (operator === 'in' || operator === 'not-in') {
                if (!Array.isArray(value) || value.length === 0) {
                    throw new Error("Invalid Query. A non-empty array is required for '".concat(operator, "' filters."));
                }
     ...
     * ```
     */ select() {
        for(var _len = arguments.length, fieldPaths = new Array(_len), _key = 0; _key < _len; _key++){
 ...
 */ class QueryOptions {
     ...
     */ static forCollectionGroupQuery(collectionId) {
        let converter = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : (0, types_1.defaultConverter)();
     ...
     */ static forKindlessAllDescendants(parent, id) {
        let requireConsistency = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : true;
 ...
 */ class DocumentReference {
     ...
     */ get() {
        return this._firestore._traceUtil.startActiveSpan(trace_util_1.SPAN_NAME_DOC_REF_GET, ()=>{
            return this._firestore.getAll(this).then((param)=>{
                let [result] = param;
                return result;
            });
        });
     ...
     */ create(data) {
        return this._firestore._traceUtil.startActiveSpan(trace_util_1.SPAN_NAME_DOC_REF_CREATE, ()=>{
            ...
            return writeBatch.create(this, data).commit().then((param)=>{
                let [writeResult] = param;
                return writeResult;
            });
        });
    }
     ...
     */ delete(precondition) {
        return this._firestore._traceUtil.startActiveSpan(trace_util_1.SPAN_NAME_DOC_REF_DELETE, ()=>{
            const writeBatch = new index_1.WriteBatch(this._firestore);
            return writeBatch.delete(this, precondition).commit().then((param)=>{
                let [writeResult] = param;
                return writeResult;
            });
        });
     ...
     */ set(data, options) {
        return this._firestore._traceUtil.startActiveSpan(trace_util_1.SPAN_NAME_DOC_REF_SET, ()=>{
            ...
            return writeBatch.commit().then((param)=>{
                let [writeResult] = param;
                return writeResult;
            });
        });
     ...
     */ update(dataOrField) {
        ...
        return this._firestore._traceUtil.startActiveSpan(trace_util_1.SPAN_NAME_DOC_REF_UPDATE, ()=>{
            ...
            const writeBatch = new index_1.WriteBatch(this._firestore);
            return writeBatch.update(this, dataOrField, ...preconditionOrValues).commit().then((param)=>{
                let [writeResult] = param;
                return writeResult;
            });
        });
 ...
 */ class DocumentTransform {
     ...
     */ toProto(serializer) {
        return Array.from(this.transforms, (param)=>{
            let [path, transform] = param;
     ...
     */ update(documentRef, dataOrField) {
        ...
        if (usesVarargs) {
            ...
            try {
                ...
                Object.entries(dataOrField).forEach((param)=>{
                    let [key, value] = param;
 ...
 */ function validatePrecondition(arg, value, options) {
    ...
    if (precondition.exists !== undefined) {
        ...
        if (typeof precondition.exists !== 'boolean') {
            throw new Error("".concat((0, validate_1.invalidArgumentMessage)(arg, 'precondition'), ' "exists" is not a boolean.\''));
        }
        if ((options === null || options === void 0 ? void 0 : options.allowedExistsValues) && options.allowedExistsValues.indexOf(precondition.exists) < 0) {
            throw new Error("".concat((0, validate_1.invalidArgumentMessage)(arg, 'precondition'), " ") + '"exists" is not allowed to have the value '.concat(precondition.exists, " ") + "(allowed values: ".concat(options.allowedExistsValues.join(', '), ")"));
        }
    }
    if (precondition.lastUpdateTime !== undefined) {
        ++conditions;
        if (!(precondition.lastUpdateTime instanceof timestamp_1.Timestamp)) {
            throw new Error("".concat((0, validate_1.invalidArgumentMessage)(arg, 'precondition'), ' "lastUpdateTime" is not a Firestore Timestamp.'));
        }
    }
    if (conditions > 1) {
        throw new Error("".concat((0, validate_1.invalidArgumentMessage)(arg, 'precondition'), " Input specifies more than one precondition."));
    }
}
/**
 ...
 */ function validateSetOptions(arg, value, options) {
    if (!(0, validate_1.validateOptional)(value, options)) {
        if (!(0, util_1.isObject)(value)) {
            throw new Error("".concat((0, validate_1.invalidArgumentMessage)(arg, 'set() options argument'), " Input is not an object."));
        }
        ...
        if ('mergeFields' in setOptions) {
            for(let i = 0; i < setOptions.mergeFields.length; ++i){
                try {
                    ...
                    throw new Error("".concat((0, validate_1.invalidArgumentMessage)(arg, 'set() options argument'), ' "mergeFields" is not valid: ').concat(err.message));
                }
            }
        }
        if ('merge' in setOptions && 'mergeFields' in setOptions) {
            throw new Error("".concat((0, validate_1.invalidArgumentMessage)(arg, 'set() options argument'), ' You cannot specify both "merge" and "mergeFields".'));
        }
    }
}
/**
 ...
 */ function validateNoConflictingFields(arg, data) {
    ...
    for(let i = 1; i < fields.length; ++i){
        if (fields[i - 1].isPrefixOf(fields[i])) {
            throw new Error("".concat((0, validate_1.invalidArgumentMessage)(arg, 'update map'), ' Field "').concat(fields[i - 1], '" was specified multiple times.'));
        }
    }
}
/**
 ...
 */ class BulkCommitBatch extends write_batch_1.WriteBatch {
    ...
    async bulkCommit() {
        ...
        return this._firestore._traceUtil.startActiveSpan(trace_util_1.SPAN_NAME_BULK_WRITER_COMMIT, async ()=>{
            ...
            let response;
            try {
                (0, logger_1.logger)('BulkCommitBatch.bulkCommit', tag, "Sending next batch with ".concat(this._opCount, " writes"));
 ...
 */ class BulkWriter {
     ...
     */ async _sendBatch(batch) {
        let flush = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : false;
        const tag = (0, util_1.requestTag)();
        ...
        if (underRateLimit) {
            ...
            (0, logger_1.logger)('BulkWriter._sendBatch', tag, "Backing off for ".concat(delayMs, " seconds"));
 ...
 */ function validateBulkWriterOptions(value) {
    ...
    if (!(0, util_1.isObject)(value)) {
        throw new Error("".concat((0, validate_1.invalidArgumentMessage)(argName, 'bulkWriter() options argument'), " Input is not an object."));
    }
    const options = value;
    ...
    if (options.throttling.maxOpsPerSecond !== undefined) {
        ...
        if (options.throttling.initialOpsPerSecond !== undefined && options.throttling.initialOpsPerSecond > options.throttling.maxOpsPerSecond) {
            throw new Error("".concat((0, validate_1.invalidArgumentMessage)(argName, 'bulkWriter() options argument'), ' "maxOpsPerSecond" cannot be less than "initialOpsPerSecond".'));
        }
    }
 ...
 */ class Transaction {
    ...
    async getBatchFn(param, opts) {
        let { documents, fieldMask } = param;
 ...
 */ function validateReadOptions(arg, value, options) {
    if (!(0, validate_1.validateOptional)(value, options)) {
        if (!(0, util_1.isObject)(value)) {
            throw new Error("".concat((0, validate_1.invalidArgumentMessage)(arg, 'read option'), " Input is not an object.'"));
        }
        const options = value;
        if (options.fieldMask !== undefined) {
            if (!Array.isArray(options.fieldMask)) {
                throw new Error("".concat((0, validate_1.invalidArgumentMessage)(arg, 'read option'), ' "fieldMask" is not an array.'));
            }
            for(let i = 0; i < options.fieldMask.length; ++i){
                try {
                    (0, path_1.validateFieldPath)(i, options.fieldMask[i]);
                } catch (err) {
                    throw new Error("".concat((0, validate_1.invalidArgumentMessage)(arg, 'read option'), ' "fieldMask" is not valid: ').concat(err.message));
                }
            }
        }
    }
     ...
     */ onQueryEnd() {
        ...
        this.writer.flush().then(async ()=>{
            ...
            if (this.lastError === undefined) {
                ...
                let error = new (__turbopack_context__.r("[project]/node_modules/google-gax/build/src/fallback.js [app-client] (ecmascript)")).GoogleError("".concat(this.errorCount, " ") + "".concat(this.errorCount !== 1 ? 'deletes' : 'delete', " ") + 'failed. The last delete failed with: ');
 ...
 */ class FirestoreAdminClient {
     ...
     */ constructor(opts, gaxInstance){
        ...
        if (typeof __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$build$2f$polyfills$2f$process$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["default"] === 'object' && 'versions' in __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$build$2f$polyfills$2f$process$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["default"]) {
            clientHeader.push("gl-node/".concat(__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$build$2f$polyfills$2f$process$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["default"].versions.node));
        } else {
 ...
 */ class EnabledTraceUtil {
    recordProjectId(projectId) {
        this.settingsAttributes["".concat(trace_util_1.ATTRIBUTE_SETTINGS_PREFIX, ".project_id")] = projectId;
        this.currentSpan().setAttributes(this.settingsAttributes);
    }
    millisToSecondString(millis) {
        return "".concat(millis / 1000, "s");
    }
    ...
    constructor(settings){
        ...
        if (settings.projectId) {
            this.settingsAttributes["".concat(trace_util_1.ATTRIBUTE_SETTINGS_PREFIX, ".project_id")] = settings.projectId;
        }
        this.settingsAttributes["".concat(trace_util_1.ATTRIBUTE_SETTINGS_PREFIX, ".database_id")] = settings.databaseId || path_1.DEFAULT_DATABASE_ID;
        ...
        this.settingsAttributes["".concat(trace_util_1.ATTRIBUTE_SETTINGS_PREFIX, ".host")] = "".concat(host, ":").concat(port);
        if (settings.preferRest !== undefined) {
            this.settingsAttributes["".concat(trace_util_1.ATTRIBUTE_SETTINGS_PREFIX, ".prefer_REST")] = settings.preferRest;
        }
        this.settingsAttributes["".concat(trace_util_1.ATTRIBUTE_SETTINGS_PREFIX, ".max_idle_channels")] = (_e = settings.maxIdleChannels) !== null && _e !== void 0 ? _e : index_1.DEFAULT_MAX_IDLE_CHANNELS;
        ...
        this.settingsAttributes["".concat(trace_util_1.ATTRIBUTE_SETTINGS_PREFIX, ".initial_retry_delay")] = this.millisToSecondString((_k = customRetrySettings === null || customRetrySettings === void 0 ? void 0 : customRetrySettings.initial_retry_delay_millis) !== null && _k !== void 0 ? _k : defaultRetrySettings.initial_retry_delay_millis);
        this.settingsAttributes["".concat(trace_util_1.ATTRIBUTE_SETTINGS_PREFIX, ".initial_rpc_timeout")] = this.millisToSecondString((_l = customRetrySettings === null || customRetrySettings === void 0 ? void 0 : customRetrySettings.initial_rpc_timeout_millis) !== null && _l !== void 0 ? _l : defaultRetrySettings.initial_rpc_timeout_millis);
        this.settingsAttributes["".concat(trace_util_1.ATTRIBUTE_SETTINGS_PREFIX, ".total_timeout")] = this.millisToSecondString((_m = customRetrySettings === null || customRetrySettings === void 0 ? void 0 : customRetrySettings.total_timeout_millis) !== null && _m !== void 0 ? _m : defaultRetrySettings.total_timeout_millis);
        this.settingsAttributes["".concat(trace_util_1.ATTRIBUTE_SETTINGS_PREFIX, ".max_retry_delay")] = this.millisToSecondString((_o = customRetrySettings === null || customRetrySettings === void 0 ? void 0 : customRetrySettings.max_retry_delay_millis) !== null && _o !== void 0 ? _o : defaultRetrySettings.max_retry_delay_millis);
        this.settingsAttributes["".concat(trace_util_1.ATTRIBUTE_SETTINGS_PREFIX, ".max_rpc_timeout")] = this.millisToSecondString((_p = customRetrySettings === null || customRetrySettings === void 0 ? void 0 : customRetrySettings.max_rpc_timeout_millis) !== null && _p !== void 0 ? _p : defaultRetrySettings.max_rpc_timeout_millis);
        this.settingsAttributes["".concat(trace_util_1.ATTRIBUTE_SETTINGS_PREFIX, ".retry_delay_multiplier")] = (_q = customRetrySettings === null || customRetrySettings === void 0 ? void 0 : customRetrySettings.retry_delay_multiplier.toString()) !== null && _q !== void 0 ? _q : defaultRetrySettings.retry_delay_multiplier.toString();
        this.settingsAttributes["".concat(trace_util_1.ATTRIBUTE_SETTINGS_PREFIX, ".rpc_timeout_multiplier")] = (_r = customRetrySettings === null || customRetrySettings === void 0 ? void 0 : customRetrySettings.rpc_timeout_multiplier.toString()) !== null && _r !== void 0 ? _r : defaultRetrySettings.rpc_timeout_multiplier.toString();
    }
}
 ...
 */ class Firestore {
     ...
     * @internal
     */ get formattedName() {
        return "projects/".concat(this.projectId, "/databases/").concat(this.databaseId);
    }
    /**
     * Gets a [DocumentReference]{@link DocumentReference} instance that
     ...
     */ requestStream(methodName, bidrectional, request, requestTag) {
        ...
        return this._retry(methodName, requestTag, ()=>{
            ...
            this._clientPool.run(requestTag, bidrectional, async (gapicClient)=>{
                ...
                this._traceUtil.currentSpan().addEvent("Firestore.".concat(methodName, ": Start"));
                try {
                    ...
                    const logStream = new stream_1.Transform({
                        ...
                        transform: (chunk, encoding, callback)=>{
                            ...
                            numResponses++;
                            if (numResponses === 1) {
                                this._traceUtil.currentSpan().addEvent("Firestore.".concat(methodName, ": First response received"));
                            } else if (numResponses % NUM_RESPONSES_PER_TRACE_EVENT === 0) {
                                this._traceUtil.currentSpan().addEvent("Firestore.".concat(methodName, ": Received ").concat(numResponses, " responses"));
                            }
                            callback();
                        }
                    });
                    ...
                    resultStream.on('end', ()=>{
                        stream.end();
                        this._traceUtil.currentSpan().addEvent("Firestore.".concat(methodName, ": Completed"), {
