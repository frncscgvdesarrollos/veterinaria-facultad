...
export default function LoginPage() {
    ...
    const handleLoginWithGoogle = async () => {
        ...
        try {
            ...
            setMessage({ type: 'error', content: 'Fallo al iniciar sesión con Google. Por favor, intenta de nuevo.' });
        }
    };
    ...
    const handleFormSubmit = async (e) => {
        ...
        if (isRegistering) {
            if (password !== confirmPassword) return setMessage({ type: 'error', content: "Las contraseñas no coinciden." });
            if (password.length < 6) return setMessage({ type: 'error', content: "La contraseña debe tener al menos 6 caracteres." });
            ...
            try {
                ...
                setMessage({ type: 'error', content: error.message || 'No se pudo crear la cuenta.' });
            }
        } else {
            try {
                ...
                setMessage({ type: 'error', content: 'Email o contraseña incorrectos.' });
            }
        }
    };
    ...
    const handlePasswordReset = async () => {
        if (!email) {
            return setMessage({ type: 'error', content: "Por favor, ingresa tu email para restablecer la contraseña." });
        }
        setMessage({ type: '', content: '' });
        ...
        try {
            ...
            if (error.code === 'auth/invalid-email') {
                ...
                setMessage({ type: 'error', content: 'No se pudo enviar el correo. Inténtalo de nuevo más tarde.' });
            }
        }
    };
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex flex-col justify-center items-center p-4">
            <div className="w-full max-w-md">
                ...
                {message.content && (
                    <div className={`${message.type === 'error' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'} p-3 rounded-md mb-4 text-center`}>
                        {message.content}
                    </div>
                )}
                ...
                <div className="bg-white shadow-xl rounded-2xl px-8 pt-6 pb-8 mb-4">
                    ...
                    <div className="text-center mt-6">
                        <button onClick={() => { setIsRegistering(!isRegistering); setMessage({ type: '', content: '' }); }} className="font-bold text-sm text-gray-600 hover:text-gray-800">
