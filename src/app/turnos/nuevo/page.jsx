...
dayjs.locale('es');
import { FaDog, FaCat, FaArrowLeft, FaCut, FaStethoscope, FaSpinner, FaCheck, FaPlus, FaTruck, FaMoneyBillWave, FaCreditCard, FaSun, FaMoon, FaExclamationCircle } from 'react-icons/fa';
import toast, { Toaster } from 'react-hot-toast';
import { verificarDisponibilidadTrasladoAction, crearTurnos, getAvailableSlotsForNewTurno } from '@/lib/actions/turnos.actions';
...
// --- FUNCIÓN AUXILIAR PARA CALCULAR LÍMITE DE FECHA ---
...
const ServiceAssignmentRow = ({ mascota, services, onToggle, configServicios }) => (
    <tr className="border-b">
        <td className="py-4 px-2 sm:px-4 font-bold"><div className="flex items-center gap-3">{mascota.especie.toLowerCase() === 'perro' ? <FaDog className="text-2xl text-gray-400" /> : <FaCat className="text-2xl text-gray-400" />}{mascota.nombre}</div></td>
        <td className="py-4 px-2 text-center">{configServicios.clinica_activa ? <input type="checkbox" checked={!!services.clinica} onChange={() => onToggle(mascota.id, 'clinica')} className="w-6 h-6 rounded text-blue-600 focus:ring-blue-500 cursor-pointer"/> : <span className="text-xs font-semibold text-red-500 bg-red-100 px-2 py-1 rounded-full">Suspendido</span>}</td>
        <td className="py-4 px-2 text-center">{configServicios.peluqueria_activa ? <input type="checkbox" checked={!!services.peluqueria} onChange={() => onToggle(mascota.id, 'peluqueria')} disabled={mascota.especie.toLowerCase() !== 'perro'} className="w-6 h-6 rounded text-green-600 focus:ring-green-500 disabled:bg-gray-200 disabled:cursor-not-allowed cursor-pointer"/> : <span className="text-xs font-semibold text-red-500 bg-red-100 px-2 py-1 rounded-full">Suspendido</span>}</td>
    </tr>
);
...
const HorarioClinicaSelector = ({ fecha, hora, onFechaChange, onHoraChange, disabledDays, modifiers, modifiersStyles, availableSlots, slotsLoading, slotsError }) => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
        <div className="flex justify-center">
             <DayPicker mode="single" selected={fecha} onSelect={(day, modifiers) => onFechaChange(day, modifiers.disabled)} locale={es} disabled={disabledDays} modifiers={modifiers} modifiersStyles={modifiersStyles} styles={{ caption: { color: '#1d4ed8' }, head: { color: '#3b82f6'} }}/>
        </div>
        <div>
            <label className="block text-sm font-bold text-gray-700 mb-4">Horario</label>
            {fecha ? (
                slotsLoading ? <div className="flex justify-center items-center h-24"><FaSpinner className="animate-spin text-2xl text-blue-500"/></div> :
                slotsError ? <div className="text-center text-red-600 bg-red-50 p-3 rounded-lg text-sm flex items-center gap-2"><FaExclamationCircle/> {slotsError}</div> :
                availableSlots.length > 0 ? <div className="grid grid-cols-3 gap-2"> {availableSlots.map(h => ( <button key={h} type="button" onClick={() => onHoraChange(h)} className={`p-3 rounded-lg text-center font-semibold transition-all duration-200 border ${hora === h ? 'bg-blue-600 text-white shadow-lg' : 'bg-white hover:bg-blue-100'}`}>{h}</button>))} </div> :
...
const HorarioPeluqueriaSelector = ({ fecha, turno, onFechaChange, onTurnoChange, disabledDays, modifiers, modifiersStyles }) => (
     <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
        <div className="flex justify-center"><DayPicker mode="single" selected={fecha} onSelect={(day, modifiers) => onFechaChange(day, modifiers.disabled)} locale={es} disabled={disabledDays} modifiers={modifiers} modifiersStyles={modifiersStyles} styles={{ caption: { color: '#16a34a' }, head: { color: '#22c55e'} }}/></div>
        <div>
            <label className="block text-sm font-bold text-gray-700 mb-4">Turno</label>
            {fecha ? <div className="flex gap-4"><button type="button" onClick={() => onTurnoChange('mañana')} className={`flex-1 p-3 rounded-lg border-2 flex items-center justify-center gap-2 ${turno === 'mañana' ? 'border-green-500 bg-green-50' : ''}`}><FaSun /> Mañana</button><button type="button" onClick={() => onTurnoChange('tarde')} className={`flex-1 p-3 rounded-lg border-2 flex items-center justify-center gap-2 ${turno === 'tarde' ? 'border-green-500 bg-green-50' : ''}`}><FaMoon /> Tarde</button></div>
...
export default function NuevoTurnoWizardPage() {
    ...
    // Clinic schedule states
    const [horarioClinica, setHorarioClinica] = useState({ fecha: undefined, hora: '' });
    const [availableClinicSlots, setAvailableClinicSlots] = useState([]);
    const [clinicSlotsLoading, setClinicSlotsLoading] = useState(false);
    const [clinicSlotsError, setClinicSlotsError] = useState(null);

    // Grooming schedule states
    ...
    const handleClinicDaySelect = async (day, disabled) => {
        if (disabled) return;
        setHorarioClinica({ fecha: day, hora: '' });
        setAvailableClinicSlots([]);
        setClinicSlotsError(null);
        if (!day) return;

        setClinicSlotsLoading(true);
        const numMascotasClinica = selectedMascotas.filter(m => motivosPorMascota[m.id]?.clinica).length;
        ...
        if (result.success) {
            setAvailableClinicSlots(result.data.horarios);
        } else {
            setClinicSlotsError(result.error || "Error al buscar horarios.");
        }
        setClinicSlotsLoading(false);
    };
    
    const handleGroomingDaySelect = (day, disabled) => {
        if (disabled) return;
        setHorarioPeluqueria({ fecha: day, turno: '' });
    };
    ...
    const handleNextStep = async () => {
        if (step === 4 && necesitaTraslado) {
            ...
            if (fechaPeluqueria && mascotasParaTraslado.length > 0) {
                 const result = await verificarDisponibilidadTrasladoAction({ fecha: dayjs(fechaPeluqueria).format('YYYY-MM-DD'), nuevasMascotas: mascotasParaTraslado });
                if (!result.success) {
    ...
    const handleConfirmarTurnos = async () => {
        setIsSubmitting(true);
        try {
            const result = await crearTurnos(user, { selectedMascotas, motivosPorMascota, specificServices, horarioClinica, horarioPeluqueria, necesitaTraslado, metodoPago, catalogoServicios });
            if (result.success) {
    ...
    const handleSpecificServiceChange = (mascotaId, motivo, serviceId) => setSpecificServices(p => ({ ...p, [mascotaId]: { ...p[mascotaId], [motivo]: serviceId } }));
    
    // Step validation logic
    const isStep1Complete = selectedMascotaIds.length > 0;
    const isStep2Complete = isStep1Complete && selectedMascotas.some(id => motivosPorMascota[id]?.clinica || motivosPorMascota[id]?.peluqueria);
    ...
    const costoTotal = useMemo(() => {
        let total = 0;
        selectedMascotas.forEach(mascota => {
            const serviciosMascota = specificServices[mascota.id];
            if (!serviciosMascota) return;

            // Costo clínica
            if (serviciosMascota.clinica) {
                const servicioInfo = catalogoServicios.clinica.find(s => s.id === serviciosMascota.clinica);
                if (servicioInfo) {
                    total += servicioInfo.precio || 0;
                }
            }

            // Costo peluquería
            if (serviciosMascota.peluqueria) {
                const servicioInfo = catalogoServicios.peluqueria.find(s => s.id === serviciosMascota.peluqueria);
                if (servicioInfo) {
                    const tamañoKey = TAMAÑO_PRECIOS_MAP[mascota.tamaño.toLowerCase()] || 'chico';
                    total += servicioInfo.precios[tamañoKey] || 0;
                }
            }
        });
        return total;
    }, [selectedMascotas, specificServices, catalogoServicios]);
        ...
        <section className="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl border mt-10 mb-20">
            ...
            <div className="p-6 sm:p-8 md:p-10">
                ...
                {step === 4 && <div><h2 className="text-2xl font-bold mb-8">Paso 4: Elige los horarios y traslado</h2><div className="space-y-8">
                    {necesitaHorarioClinica && <div className="bg-gray-50 p-6 rounded-xl border"><h3 className="text-xl font-bold text-gray-800 mb-5 flex items-center gap-3"><FaStethoscope className="text-blue-500"/>Turno de Clínica</h3><HorarioClinicaSelector fecha={horarioClinica.fecha} hora={horarioClinica.hora} onFechaChange={handleClinicDaySelect} onHoraChange={(hora) => setHorarioClinica(p => ({ ...p, hora }))} disabledDays={disabledDays} modifiers={modifiers} modifiersStyles={modifiersStyles} availableSlots={availableClinicSlots} slotsLoading={clinicSlotsLoading} slotsError={clinicSlotsError} /></div>}
                    {necesitaHorarioPeluqueria && <div className="bg-gray-50 p-6 rounded-xl border"><h3 className="text-xl font-bold text-gray-800 mb-5 flex items-center gap-3"><FaCut className="text-green-500"/>Turno de Peluquería</h3><HorarioPeluqueriaSelector fecha={horarioPeluqueria.fecha} turno={horarioPeluqueria.turno} onFechaChange={handleGroomingDaySelect} onTurnoChange={(turno) => setHorarioPeluqueria(p => ({...p, turno}))} disabledDays={disabledDays} modifiers={modifiers} modifiersStyles={modifiersStyles} /></div>}
                    <div className="p-4 border rounded-lg"><div className="flex items-center justify-between"><div className="flex items-center"><FaTruck className="text-2xl text-gray-600 mr-3"/><span className="font-bold text-gray-700">¿Necesitas traslado?</span></div><button type="button" onClick={() => setNecesitaTraslado(!necesitaTraslado)} className={`relative inline-flex items-center h-6 rounded-full w-11 ${necesitaTraslado ? 'bg-blue-600' : 'bg-gray-300'}`}><span className={`inline-block w-4 h-4 transform bg-white rounded-full transition-transform ${necesitaTraslado ? 'translate-x-6' : 'translate-x-1'}`}/></button></div></div>
                </div></div>}
                
                {step === 5 && <div><h2 className="text-2xl font-bold mb-8">Paso 5: Detalles Finales y Confirmación</h2><div className="space-y-8">
                    <div className="p-6 bg-gray-50 rounded-xl border">
                        <h3 className="text-xl font-bold mb-4 border-b pb-2">Resumen de Turnos</h3>
                        <div className="space-y-4">
                        {selectedMascotas.map(mascota => {
                            const servicios = specificServices[mascota.id] || {};
                            const tieneServicios = servicios.clinica || servicios.peluqueria;
                            if (!tieneServicios) return null;
                            
                            const getServicioInfo = (motivo, serviceId) => catalogoServicios[motivo].find(s => s.id === serviceId);
                            const getPrecio = (motivo, servicioInfo) => {
                                if (!servicioInfo) return 0;
                                if (motivo === 'clinica') return servicioInfo.precio || 0;
                                const tamañoKey = TAMAÑO_PRECIOS_MAP[mascota.tamaño.toLowerCase()] || 'chico';
                                return servicioInfo.precios[tamañoKey] || 0;
                            };

                            const clinicaInfo = servicios.clinica ? getServicioInfo('clinica', servicios.clinica) : null;
                            const peluqueriaInfo = servicios.peluqueria ? getServicioInfo('peluqueria', servicios.peluqueria) : null;
                            const clinicaPrecio = getPrecio('clinica', clinicaInfo);
                            const peluqueriaPrecio = getPrecio('peluqueria', peluqueriaInfo);

                            return (
                                <div key={mascota.id} className="border-b last:border-b-0 pb-3 mb-3">
                                    <p className="font-bold text-lg text-gray-800">{mascota.nombre}</p>
                                    {clinicaInfo && <div className="flex justify-between items-center text-sm"><p>{clinicaInfo.nombre}</p><p className="font-semibold">${clinicaPrecio.toFixed(2)}</p></div>}
                                    {peluqueriaInfo && <div className="flex justify-between items-center text-sm"><p>{peluqueriaInfo.nombre}</p><p className="font-semibold">${peluqueriaPrecio.toFixed(2)}</p></div>}
                                </div>
                            );
                        })}
                        </div>
